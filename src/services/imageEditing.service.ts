 ...
 * 笔刷遮罩编辑功能：使用遮罩局部编辑图片
 * @param originalImageUrl - 原始图片的URL
 * @param maskData - 遮罩数据（base64格式）
 * @param editInstruction - 编辑指令
 * @param sessionId - 可选的会话ID，用于跟踪设计过程
 * @returns - 返回编辑后的设计方案数据
 */
export async function brushEditImage(originalImageUrl: string, maskData: string, editInstruction: string, sessionId?: string): Promise<DesignData> {
  console.log('开始笔刷遮罩编辑图片，会话ID:', sessionId);
  
  try {
    // 调用新的笔刷编辑 Edge Function
    ...
    if (error) {
      throw new Error(error.message || "Failed to brush edit image.");
    }
    
    if (!data.success) {
      throw new Error(data.error || "Failed to brush edit image.");
    }
    
    // 构造返回的设计数据
    const designData: DesignData = {
      url: `data:image/png;base64,${data.imageData}`,
      prompt_en: editInstruction,
      design_name: '笔刷编辑后设计'
    };
      ...
      try {
        ...
        if (latestBrief) {
          ...
          await sessionService.addGeneratedImage(
            ...
            expandedPrompt.id,
            designData.url,
            designData.design_name || '笔刷编辑后设计',
            undefined, // messageId
            'success' // status as literal
          );
        }
        
        console.log('笔刷遮罩编辑过程已记录到数据库');
      } catch (dbError) {
        console.error('记录笔刷遮罩编辑过程到数据库失败:', dbError);
      }
    }
    
    return designData;
  } catch (error) {
    console.error('笔刷遮罩编辑图片失败:', error);
      ...
      try {
        ...
        if (latestBrief) {
          ...
          await sessionService.addGeneratedImage(
            sessionId,
            expandedPrompt.id,
            'https://placehold.co/1024x1024/f87171/ffffff?text=Brush+Edit+Failed',
            '笔刷编辑失败',
            undefined, // messageId
            'failed', // status as literal
            error instanceof Error ? error.message : '未知错误'
        ...
        console.error('记录笔刷编辑失败状态到数据库失败:', dbError);
      }
    }
